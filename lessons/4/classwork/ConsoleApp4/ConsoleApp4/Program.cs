    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (Regex.IsMatch(a, "^[0-9]*[,]?[0-9]+$") && Regex.IsMatch(h, "^[0-9]*[,]?[0-9]+$"))
            {
                ...
                var result2 = (Math.Sqrt(3)*doubleA + 2*doubleH)*(3/2)*doubleA;
                Console.WriteLine("S2 = " + result2);
                var result3 = Math.Sqrt(Math.Pow(doubleH,2) - (Math.Pow(doubleA,2)/12));
                Console.WriteLine("H = " + Math.Round(result3));
                var result4 = Math.Pow(doubleA,2)/2*result3*Math.Sqrt(doubleH);
